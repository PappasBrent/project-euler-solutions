...
# Empty list for storing the input values; will be a 2D array
# or grid
triangle = []

# Loads the file for reading
triangleFile = open("18.txt", "r")

# Turns each line into a list of ints, then adds them
# to the triangle grid
for s in triangleFile.readlines():
    triangle.append(list(map(int, s.split())))

# Closes the file just because
triangleFile.close()
